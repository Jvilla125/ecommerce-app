...
const CategoryFilterComponent = ({ setCategoriesFromFilter }) => {
  ...
  const selectCategory = (e, category, idx) => {
    ...
    var selectedMainCategory = category.name.split("/")[0]; // gets the first category in the string
    var allCategories = myRefs.current.map((_, id) => {
      return { name: categories[id].name, idx: id }; // each category will have their own id
    });
    ...
    <>
      ...
      <Form>
        {categories.map((category, idx) => (
          <div key={idx} >
            {/* changed check-api2 bc rating already has same idx with check-api */}
            <Form.Check type="checkbox" id={`check-api2-${idx}`}>
              <Form.Check.Input ref={(el) => (myRefs.current[idx] = el)} type="checkbox" isValid
